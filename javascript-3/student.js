...
let groceries = ['banana', 'apple', 'chopsticks']
...
function checkGroceryCount(){
    return groceries.length >= 5 ? 'That looks like a big trip'
    : groceries.length === 1 ? '1 item'
    : groceries.length > 1 && groceries.length < 5 ? `${groceries.length} items`
    : null
}
